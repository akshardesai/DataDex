...
import { deleteLogDB } from '../../utils/DailyLogs';

export default function DeleteBtn({member}) {
    const [isOpen, setIsOpen] = useState(false);
    
    async function deleteMemberSubmit(e) {
        e.preventDefault();
        const response = await deleteLogDB(member)
        if (response.success) {
            console.log('delete log db');
            
        } else {
            alert(`Error deleting log ${response.error}`)
        }
    }
                ...
                >
                    <div className="mb-4 p-4">
                        <p className="text-white">
                            Delete Member Permanently ? Are you sure.. This action cannot be
                            undone
                        </p>
                    </div>
                </Modal>
