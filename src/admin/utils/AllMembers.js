...
export async function generateCodeDB(member) {
  try {
    console.log("member id to generate code -> ", member.id);

    const code = Math.floor(10000 + Math.random() * 90000).toString();

    const documentRef = doc(membersCollectionRef, member.id);

    await updateDoc(documentRef, { code });

    return { success: true, data: code };
  } catch (error) {
    return { success: false, error: `Error generating code ${error}` };
  }
}

export async function logoutUserDB(member) {
  try {
    const loginStatus = false;
    const documentRef = doc(membersCollectionRef, member.id);
    await updateDoc(documentRef, { loginStatus });

    return { success: true };
  } catch (error) {
    return { success: false, error: `Error updating login status ${error}` };
  }
}
...
export async function editMembershipDB(memberid, info, updatedMembershipData) {
  try {
    ...
    const indexToUpdate = allMemberships.findIndex(
